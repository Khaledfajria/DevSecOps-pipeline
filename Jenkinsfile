        ...
        stage('SonarQube') {
            steps {
                //withEnv(["SONAR_TOKEN=${env.SONAR_TOKEN}"]) {
                    echo 'pass'
                    sh "$HOME/.sonar/sonar-scanner-4.7.0.2747-linux/bin/sonar-scanner -Dsonar.projectKey=django-eco -Dsonar.sources=. -Dsonar.host.url=https://9000-port-327a81bcc3a441c5.labs.kodekloud.com -Dsonar.login=sqp_7bd835e2b2e8937e781dd4c843360e76eabf1a98"
                //}
            }
        }
